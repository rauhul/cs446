...
class LinearMRF(object):
    ...
    def get_unary_features(self, img):
        ...
        return np.vstack((img == 0, img == 1)).T
    ...
    def get_pairwise_features(self):
        ...
        plen = len(self.pairs)
        ones = np.ones((plen, 1))
        zeros = np.zeros((plen, 1))

        return np.hstack((ones, zeros, zeros, ones))
    ...
    def build_training_obj(self, img_features, unary_beliefs, pairwise_beliefs,
        ...
        Args:
            ...
            pairwise_beliefs(tf.Tensor): A list of the pairwise beliefs for each
        ...
        imgu_potentials = tf.reduce_sum(self.calculate_unary_potentials(img_features))
        imgp_potentials = tf.reduce_sum(self.calculate_pairwise_potentials(self.get_pairwise_beliefs(img_features)))
        img_term = imgu_potentials + imgp_potentials

        unary_term = tf.reduce_sum(tf.stack(unary_beliefs) * unary_potentials)
        pair_term = tf.Variable(0.)
        for p in pairwise_beliefs:
            pair_term += p * pairwise_potentials
        pair_term = tf.reduce_sum(pair_term)

        return unary_term + pair_term - img_term
    ...
    def inference_itr(self, unary_beliefs, unary_pots, pairwise_pots):
        ...
        idx = np.arange(len(unary_beliefs))
        np.random.shuffle(idx)
        for i in idx:
            zero = self.calculate_local_score(i, 0, unary_beliefs, unary_pots, pairwise_pots)
            one  = self.calculate_local_score(i, 1, unary_beliefs, unary_pots, pairwise_pots)
            if zero > one:
                unary_beliefs[i] = [1, 0]
            else:
                unary_beliefs[i] = [0, 1]
    ...
    def calculate_local_score(self, node, assignment, unary_beliefs,
        ...
        for neighbor in self.neighbors[node]:
            m = assignment
            n = unary_beliefs[neighbor, 1]
            idx = m*2 + n
            score += pairwise_potentials[self.pair_inds[(neighbor, node)], idx]
            score += n
    ...
    def check_convergence(self, new_unary_beliefs, old_unary_beliefs,
        ...
        return (new_unary_beliefs == old_unary_beliefs).mean() > convergence_margin
    ...
    def get_pairwise_beliefs(self, unary_beliefs):
        ...
        for index, pair in zip(range(len(self.pairs)), self.pairs):
            a, b = unary_beliefs[pair[0], 1], unary_beliefs[pair[1], 1]
            i = (int) ((a * 2) + b)
            result[index, i] = 1
