...
class VariationalAutoencoder(object):
    ...
    def _sample_z(self, z_mean, z_log_var):
        ...
        epsilon = tf.random_normal(tf.shape(sigma), mean=0.0, stddev=1.0)
        return mu + tf.multiply(sigma, epsilon)
    ...
    def _encoder(self, x):
        ...
        Returns:
            ...
            z_log_var(tf.Tensor): The latent log variance, tensor of dimension
                (None, _nlatent).
        """
        input_l   = fully_connected(x,        100,           activation_fn=tf.nn.softplus)
        hidden_l  = fully_connected(input_l,  50,            activation_fn=tf.nn.softplus)
        z_mean    = fully_connected(hidden_l, self._nlatent, activation_fn=None)
        z_log_var = fully_connected(hidden_l, self._nlatent, activation_fn=None)
    ...
    def _decoder(self, z):
        ...
        """
        input_l  = fully_connected(z,        50,          activation_fn=tf.nn.softplus)
        hidden_l = fully_connected(input_l,  100,         activation_fn=tf.nn.softplus)
        final_l  = fully_connected(hidden_l, self._ndims, activation_fn=tf.nn.sigmoid)
        return final_l
    ...
    def _latent_loss(self, z_mean, z_log_var):
        ...
        """
        sigma_2     = tf.exp(z_log_var)
        latent_loss = 0.5 * tf.reduce_sum( - z_log_var + tf.square(z_mean) + sigma_2 - 1)
        return latent_loss
