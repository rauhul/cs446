...
class GameState:
    ...
    def frame_step(self,input_vect):
        ...
        # Key up.
        if input_vect[1] == 1:
            self.bar1_move = -self.speed_multiplier * ai_speed
        # Key down.
        elif input_vect[2] == 1:
            self.bar1_move = self.speed_multiplier * ai_speed
        # Don't move.
        ...
        if self.circle_x >= 305.:
            if not self.bar2_y == self.circle_y + 7.5:
                if self.bar2_y < self.circle_y + 7.5:
                    self.bar2_y += self.speed_multiplier * ai_speed
        ...
        self.circle_x += self.speed_multiplier * self.speed_x
        self.circle_y += self.speed_multiplier * self.speed_y
